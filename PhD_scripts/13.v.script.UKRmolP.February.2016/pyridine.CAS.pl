...
%personal = (
  ...
  'ad_email_from', 'agnieszka.sieradzka@open.ac.uk', 		# here the e-mail address can be given to send information about the end of calculations; 
  ...
  'archer_login',  "agaa\@login.archer.ac.uk",  			# the login to archer machine to run there evething except molpro
...
for (my $rrr = 2.4; $rrr <= 2.4; $rrr += 0.1) {
  push(@{$geometry{'geometries'}},
       { 'description', sprintf("%8.5f", $rrr),
         'natype', 7,
         'atoms', [ [ "N",  0.0,                          0.0,                                2.696450216925348 ],
                    [ "C", 0.0,                      -2.157500324243935,                      1.366649938244034 ],
                    [ "C", 0.0,                      -2.266726494640098, 	             -1.275187193483233 ],
                    [ "C", 0.0,                          0.0,                                -2.628609048807316 ],
                    [ "H", 0.0,          	     -3.891324049823167, 		      2.474029451257037 ],
                    [ "H", 0.0,	                     -4.081997416483597, 		     -2.232900396887653 ],
                    [ "H", 0.0,                          0.0,                                -4.682552381049352 ],
                    [ "C", 0.0,                       2.266726494640098, 	             -1.275187193483233 ],
                    [ "C", 0.0,                       2.157500324243935, 	              1.366649938244034 ],
                    [ "H", 0.0,                       4.081997416483597,                     -2.232900396887653 ],
                    [ "H", 0.0, 		      3.891324049823167, 		      2.474029451257037 ] ]
...
%model = ( 
  ...
  # Molecule
  'molecule',   #"C5H5N",		# Used only for directory and descriptions  
 		#"pyridine_UKRmol",
		"pyridine",
       
  'atoms',        ["N", "C", "C", "C", "H", "H", "H", "C", "C", "H", "H"],    # All atoms must be specified
  'nelectrons',    42,             	# number of target electrons
  ...
  'model',       "CAS",             	# options are "SE"    = static-exchange, 
                 #"SEP",             	#             "SEP"   = SE + polarization, 
                 #"SE",            	#             "CAS"   = complete active space (as a default it is used the model B below), 
  ...
  'nfrozen',       "16", #16           	# number of frozen target orbitals (used for SEP and CAS)      
  'nactive',       "8",  #8           	# number of active target orbitals (used for SEP and CAS)
  'nvirtual',      "18",             	# number of virtual states
...
#  'virtual_orbs',  [0,0,0,0,0,0,0,0], 	# which orbitals for each symmetry to use as virtual,
  'frozen_orbs',   [10,0,6,0,0,0,0,0], # which orbitals for each symmetry to use as frozen, (also used in MOLPRO model CAS)
  'active_orbs',   [1,4,1,2,0,0,0,0], 	# which orbitals for each symmetry to use as active, (also used in MOLPRO model CAS /MOLPRO input: occ = frozen_orbs + active_orbs/)
#  'frozen_orbs',   [10,1,6,0,0,0,0,0], # which orbitals for each symmetry to use as frozen, (also used in MOLPRO model CAS)
#  'active_orbs',   [1,3,1,2,0,0,0,0], 	# which orbitals for each symmetry to use as active, (also used in MOLPRO model CAS /MOLPRO input: occ = frozen_orbs + active_orbs/)
#  'frozen_orbs',   [11,1,7,0,0,0,0,0], # which orbitals for each symmetry to use as frozen, (also used in MOLPRO model CAS)
#  'active_orbs',   [3,3,2,2,0,0,0,0], 	# which orbitals for each symmetry to use as active, (also used in MOLPRO model CAS /MOLPRO input: occ = frozen_orbs + active_orbs/)
#  'virtual_orbs',  [0,0,0,0,0,0,0,0], 	# which orbitals for each symmetry to use as virtual,
  'virtual_orbs',  [8,2,7,1,0,0,0,0], 	# which orbitals for each symmetry to use as virtual,
#  'virtual_orbs',  [10,2,7,1,0,0,0,0],	# 20 virtual, 
#  'virtual_orbs',  [17,6,13,4,0,0,0,0],# 40 virtual, 
#  'virtual_orbs',  [20,8,16,6,0,0,0,0],# 50 virtual,
#  'virtual_orbs',  [24,10,19,7,0,0,0,0],# 60 virtual, 
#  'virtual_orbs',  [27,11,23,9,0,0,0,0],# 70 virtual,
#  'virtual_orbs',  [31,13,26,10,0,0,0,0],# 80 virtual, 

  # Input for MOLPRO 
  'molpro_setting',    "", 
  'ncasscf_states',  {'singlet', [1,1,1,1,0,0,0,0],  	# number of states used to optimalised orbitals in MOLPRO
		      'triplet', [2,1,1,1,0,0,0,0]}, 	# Warning! It works only for 'singlet', 'triplet'.
...
#		      'triplet', [2,2,2,2,0,0,0,0]}, 	# Warning! It works only for 'singlet', 'triplet'.
  ...
  'ntarget_states', {'singlet', [1,1,1,1,0,0,0,0],  	# number of target states to calculate in each irreducible representation (IR) for CAS
                     'triplet', [2,1,1,1,0,0,0,0]},
...
#                     'triplet', [0,0,0,0,0,0,0,0]},
  'ntarget_states_used', "9",         			# number of target states which will be actually used in scattering calculations 
  ...
  'L_basis',	   "6",		      	# The maximum of partial waves included into continuum
  ...
  'nescat',        "600", #800   		# number of input scattering energies in each subrange (input for R_SOLVE via namelist &rslvin)
  ...
  # Input for RADDEN
  'radius_start',  "0",              	# radial interval in a.u. for which the integration will be carried out
  'radius_finish', "20",              	# the last value of radial for which the integration will be calculated
  'req_dm',	   "-1",          	# if < 0 (e.g. -1) to all available orbitals/density matrices will be read in and integrated, otherwise the number of orbitals/density matrices to be read in from the density matrix file
  'dm_list',	   "0",          	# first density matrices/orbitals to be read in from the input file. Not used if ilist .eq. 'false' (for non-sequential orbtials, modify input manually)
...
%run = (
  ...
  'suffix',          "_prec.CAS10_8",  		# string added to model directory to distinguish different runs which have the same model and number of orbitals and states
...
#  'parallel_geom',   1,  		# number of geometries to be invoked in the same time
  'parallel_symm',   4,  		# number of symmetries to be invoked in the same time
...
#  'only',           "target-congen|target-scatci|target-denprop|scattering-congen|scattering-scatci|scattering-outerres|scattering-time-delay", #"target-molpro", # target-congen|target-scatci|target-denprop|scattering-congen|scattering-scatci|  scattering-hamdiag| "target-mpoutrd|target-swedmos|target-swtrmo|target-congen|target-scatci|target-gausprop|target-denprop|scattering-swmol3|scattering-gaustail|scattering-sword|scattering-swedmos|scattering-swtrmo|scattering-congen|scattering-scatci|scattering-hamdiag|scattering-outerres"      # e.g. "scattering-congen|scattering-scatci|scattering-outerres" to run only last part of the scastering calculation

  'no_processors',   $ARGV[1],  	# number of processors; it only applies for integrals or hamdiag programs
  'computer', 	     2,  	 	# 0 - uqbar, 1 - impact cluster, 2 - archer
...
%dirs = (
  ...
  'bin_out',   "/padata/gamma/users/asieradzka/bin_UKRmol_$run{'precision'}",     # /padata/beta/users/rmatrix/CCPForge/ukrmol-out/trunk/bin
  'integrals', "/padata/gamma/users/asieradzka/bin_UKRmol_$run{'precision'}",     # quadruple precision binaries for UKRmol+
