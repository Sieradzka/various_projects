...
for (my $rrr = 2.4; $rrr <= 2.4; $rrr += 0.1) {
  push(@{$geometry{'geometries'}},
       { 'description', sprintf("%8.5f", $rrr),
         'natype', 17,
         'atoms', [ [ "H",    1.765597114,   -2.881771070,   -6.644236369 ],
                    [ "O",    0.088350845,   -2.582562060,   -6.099313499 ],
                    [ "H",   -0.055249669,   -0.785659150,   -6.067281131 ],
                    [ "O",   -0.109404427,    2.809817332,   -5.093569118 ],
                    [ "H",   -1.718698310,    3.567606287,   -5.284759872 ],
                    [ "H",    0.099923121,    2.617867686,   -3.308090634 ],
		    [ "N",    0.075385341,    1.260555863,    0.314900347 ],
                    [ "C",    0.168909669,    2.426371508,    2.527446084 ],
                    [ "H",    0.307091222,    4.467843952,    2.469900893 ],
                    [ "C",    0.101040778,    1.172815137,    4.824631207 ],
                    [ "H",    0.183695190,    2.221864743,    6.575031817 ],
                    [ "C",   -0.072671212,   -1.442895405,    4.801245654 ],
                    [ "H",   -0.130739155,   -2.497617517,    6.552524112 ],
                    [ "C",   -0.168841376,   -2.677320220,    2.495119272 ],
                    [ "H",   -0.304030109,   -4.711732230,    2.379775389 ],
                    [ "C",   -0.087918777,   -1.240281229,    0.302165152 ],
                    [ "H",   -0.152182796,   -2.134401766,   -1.537294285 ] ]
       }
  );
}
...
%model = ( 
  ...
  # Molecule
  'molecule',     "pyridine_2water",	# Used only for directory and descriptions  

       
  'atoms',        ["H", "O", "H", "O", "H", "H", "N", "C", "H", "C", "H", "C", "H", "C", "H", "C", "H"],    # All atoms must be specified
  'nelectrons',    62,             	# number of target electrons
  ...
  'nfrozen',       "16",            	# number of frozen target orbitals (used for SEP and CAS)      
  'nactive',       "15",             	# number of active target orbitals (used for SEP and CAS)
  'nvirtual',      "30",             	# number of virtual states
...
#                     'triplet', [4,4,4,4,0,0,0,0]},
  ...
  'req_dm',	   "54",          	# if < 0 (e.g. -1) all available orbitals/density matrices will be read in and integrated, otherwise the number of orbitals/density matrices to be read in from the density matrix file
  'dm_list',	   "17",          	# first density matrices/orbitals to be read in from the input file. Not used if ilist .eq. 'false' (for non-sequential orbtials, modify input manually)
...
%run = (
  ...
  'precision',       "quadru", 		# "double" or "quadru" - option whether run executables with double or quadruple precision. Leave empty if you do not need
