...
%personal = (
  ...
  'archer_login',  "agaa\@login.archer.ac.uk",  			# the login to archer machine to run there everything except molpro
...
for (my $rrr = 2.4; $rrr <= 2.4; $rrr += 0.1) {
  push(@{$geometry{'geometries'}},
       { 'description', sprintf("%8.5f", $rrr),
         'natype', 10,
         'atoms', [ [ "N", -0.003798821,    1.054544304,     0.000000000],
                    [ "C", -0.002812296,   -0.233512747,   -2.143369400],
                    [ "C",  0.003094782,   -2.851460035,   -2.253989080 ],
                    [ "C",  0.006846765,   -4.186532116,    0.000000000 ],
                    [ "H", -0.009538994,    0.876759039,   -3.862745240],
                    [ "H",  0.002543472,   -3.806924597,   -4.059117560],
                    [ "H", 0.010697790,   -6.231625376,    0.000000000],
                    [ "H", -0.232640284,    5.051865559,    0.000000000], # Hydrogen bond
                    [ "O", -0.092578842,    6.845060685,    0.000000000], # water
                    [ "H", 1.669940030,    7.143374634,     0.000000000], # water
                    [ "C", -0.002812296,   -0.233512747,    2.143369400 ],
                    [ "C", 0.003094782,   -2.851460035,    2.253989080],
                    [ "H", 0.002543472,   -3.806924597,    4.059117560 ],
                    [ "H", -0.009538994,    0.876759039,    3.862745240 ] ]
...
%model = ( 
  ...
  # Molecule
  'molecule',     "pyridine_water",	# Used only for directory and descriptions  

       
  'atoms',        ["N", "C", "C", "C", "H", "H", "H", "H", "O", "H", "C", "C", "H", "H"],    # All atoms must be specified
  'nelectrons',    52,             	# number of target electrons
  ...
  'nfrozen',       "20", #14 7 19     	# number of frozen target orbitals (used for SEP and CAS)      
  'nactive',       "9", #12 19 10      	# number of active target orbitals (used for SEP and CAS)
  'nvirtual',      "20",  #30 55       	# number of virtual states
...
#  'frozen_orbs',   [13,6,0,0,0,0,0,0], 	# which orbitals for each symmetry to use as frozen, (also used in MOLPRO model CAS)
#  'active_orbs',   [6,4,0,0,0,0,0,0], 	# which orbitals for each symmetry to use as active, (also used in MOLPRO model CAS /MOLPRO input: occ = frozen_orbs + active_orbs/) 
#  'virtual_orbs',  [5,3,0,0,0,0,0,0], 	# which orbitals for each symmetry to use as virtual,
  'frozen_orbs',   [14,6,0,0,0,0,0,0], 	# which orbitals for each symmetry to use as frozen, (also used in MOLPRO model CAS)
  'active_orbs',   [5,4,0,0,0,0,0,0], 	# which orbitals for each symmetry to use as active, (also used in MOLPRO model CAS /MOLPRO input: occ = frozen_orbs + active_orbs/) 
#  'virtual_orbs',  [5,2,0,0,0,0,0,0], 	# 7 virtual,
  'virtual_orbs',  [12,8,0,0,0,0,0,0], 	# 20 virtual,

  # Input for MOLPRO 
  'molpro_setting',    "", 
  'ncasscf_states',  {'singlet', [2,2,0,0,0,0,0,0],  	# number of states used to optimalised orbitals in MOLPRO
		      'triplet', [3,2,0,0,0,0,0,0]}, 	# Warning! It works only for 'singlet', 'triplet'.
  ...
  'ntarget_states', {'singlet', [2,2,0,0,0,0,0,0],  	# number of target states to calculate in each irreducible representation (IR) for SEP
                     'triplet', [3,2,0,0,0,0,0,0]},
  ...
  'max_multipole', "1",               	# maximum multipole to be retained in expansion of long range potentials
...
%run = (
  ...
  'suffix',          "_prec.CAS12_9",  		# string added to model directory to distinguish different runs which have the same model and number of orbitals and states
...
#  'parallel',	     0,  		# 1 for run hamdiag - parallel diagonalization of Hamiltionian matrix
  ...
  'only',             "scattering-congen|scattering-scatci|scattering-outerres|scattering-time-delay|scattering-timedel",#"target-congen|target-scatci|target-denprop|scattering-congen|scattering-scatci|scattering-outerres|scattering-time-delay", # scattering-hamdiag| "target-mpoutrd|target-swedmos|target-swtrmo|target-congen|target-scatci|target-gausprop|target-denprop|scattering-swmol3|scattering-gaustail|scattering-sword|scattering-swedmos|scattering-swtrmo|scattering-congen|scattering-scatci|scattering-hamdiag|scattering-outerres"      # e.g. "scattering-congen|scattering-scatci|scattering-outerres" to run only last part of the scastering calculation
