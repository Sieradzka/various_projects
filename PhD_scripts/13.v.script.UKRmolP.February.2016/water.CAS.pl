...
%personal = (
  ...
  'archer_script', "-q short archer.run.pbs",  			# name of a script to submit on archer; to submit into short queue: "-q short name_of_script"
...
for (my $rrr = 2.4; $rrr <= 2.4; $rrr += 0.1) {
  push(@{$geometry{'geometries'}},
       { 'description', sprintf("%8.5f", $rrr),
         'natype', 3,
#         'atoms', [ [ "H", 0.85007558,    6.47787515, 0.00000000 ], # Hydrogen bond
#                    [ "O", 1.06658418,    8.26113990, 0.00000000 ], # water
#                    [ "H", 2.82136489,    8.56481383, 0.00000000 ]] # water
#         'atoms', [ [ "H", -0.370335811,    -1.635305668, 0.00000000 ], # Hydrogen bond
#                    [ "O", -0.153827211,    0.147959082, 0.00000000 ], # water
#                    [ "H", 1.600953499,     0.451633012, 0.00000000 ]] # water
         'atoms', [ [ "O", 0.0,                      0.0,                      0.1157 ],
                    [ "H", 0.0,                      0.7488,                  -0.4628 ],
                    [ "H", 0.0, 		    -0.7488, 		      -0.4628] ]
       }
  );
...
%model = ( 
  ...
  # Molecule
  'molecule',     "water",		# Used only for directory and descriptions  
  ...
  # Units
  'r_unit',        1,                 	# distance unit: 0 - atomic units, 1 - Angstroms
  ...
  'nfrozen',       "3", #2           	# number of frozen target orbitals (used for SEP and CAS)      
  'nactive',       "5", #4            	# number of active target orbitals (used for SEP and CAS)
...
#  'frozen_orbs',   [2,0,0,0,0,0,0,0], 	# which orbitals for each symmetry to use as frozen, (also used in MOLPRO model CAS)
#  'active_orbs',   [2,1,1,0,0,0,0,0], 	# which orbitals for each symmetry to use as active, (also used in MOLPRO model CAS /MOLPRO input: occ = frozen_orbs + active_orbs/) 
#  'virtual_orbs',  [0,0,0,0,0,0,0,0], 	# which orbitals for each symmetry to use as virtual,
  'frozen_orbs',   [2,1,0,0,0,0,0,0], 	# which orbitals for each symmetry to use as frozen, (also used in MOLPRO model CAS)
  'active_orbs',   [3,0,1,1,0,0,0,0], 	# which orbitals for each symmetry to use as active, (also used in MOLPRO model CAS /MOLPRO input: occ = frozen_orbs + active_orbs/) 
  'virtual_orbs',  [0,0,0,0,0,0,0,0], 	# which orbitals for each symmetry to use as virtual,

  # Input for MOLPRO 
  'molpro_setting',    "", 
  'ncasscf_states',  {'singlet', [1,1,0,0,0,0,0,0],  	# number of states used to optimalised orbitals in MOLPRO
		      'triplet', [0,1,0,0,0,0,0,0]}, 	# Warning! It works only for 'singlet', 'triplet'.
  ...
  'delthres',      "1.0D-14, 1.0D-14, 1.0D-14, 1.0D-14",# deletion threshold in swedmos                                    

  # Target states: for closed-shell target, specify the number of singlets, triplets, ...
  #                for   open-shell target,                       doublets, quartets, ...   
  'ntarget_states', {'singlet', [2,1,1,1,0,0,0,0],  	# number of target states to calculate in each irreducible representation (IR) for CAS
                     'triplet', [1,1,1,1,0,0,0,0]},
...
#                     'triplet', [0,0,0,0,0,0,0,0]},
  ...
  'dm_list',	   "0",          	# sequence numbers of density matrices/orbitals to be read in from the input file. Not used if ilist .eq. 'false'
...
%run = (
  ...
  'suffix',          "_prec.CAS4_5",  		# string added to model directory to distinguish different runs which have the same model and number of orbitals and states
  ...
  'precision',       "quadru", 		# "double" or "quadru" - option whether run executables with double or quadruple precision
